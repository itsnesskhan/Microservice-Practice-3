...
//  direct call, no load balancing
	...
	public ApiResponse getAllOrderOfUserUsingRestTemplate(Integer id) {
		...
		List<OrderDTO> orderDtosWithProduct = ordersDtos.stream().map(order->{
			ApiResponse apiResponse = restTemplate.getForObject("http://localhost:8081/product-app/api/product/{id}", ApiResponse.class, order.getProductId());
...
//	using rest template
//  direct call, load balancing
	@Override
	public ApiResponse getAllOrderOfUserUsingRestTemplateLoadBalanced(Integer id) {
		List<Order> orders = orderRepository.findByUserId(id);
		List<OrderDTO> ordersDtos = Arrays.asList(modelMapper.map(orders, OrderDTO[].class));
		
		List<OrderDTO> orderDtosWithProduct = ordersDtos.stream().map(order->{
			ApiResponse apiResponse = restTemplate.getForObject("http://PRODUCT-SERVICE/product-app/api/product/{id}", ApiResponse.class, order.getProductId());
			ProductDTO productDTO = objectMapper.convertValue(apiResponse.getData(), ProductDTO.class);
			order.setProduct(productDTO);
			return order;
		}).collect(Collectors.toList());
...
//	direct call no load balancing
	...
	public ApiResponse getAllOrderOfUserUsingFeignClient(Integer id) {
		...
		List<OrderDTO> orderDtosWithProduct = ordersDtos.stream().map(order->{
			ApiResponse apiResponse= productClient.getProductById(order.getProductId()).getBody();
			ProductDTO productDTO = objectMapper.convertValue(apiResponse.getData(), ProductDTO.class);
			order.setProduct(productDTO);
			return order;
		}).collect(Collectors.toList());
